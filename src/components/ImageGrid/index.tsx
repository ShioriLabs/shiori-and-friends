...
const ImageGridItem = ({ document }: Props): React.ReactElement => {
  return (
    <article className="w-full rounded-lg relative max-h-[512px] border border-borderColor">
      <Image src={document.data.images[0].image.url} alt={`${document.data.title ?? 'Gambar Shiori'} oleh ${document.data.author_name ?? 'Anonim'}`} width={document.data.images[0].image.dimensions.width} height={document.data.images[0].image.dimensions.height} className="object-cover w-full h-full rounded-lg" />
